...
from main import find_collocation_errors, find_collocation_errors_vec, suggest_corrections, check_collocation_frequency
...
def check_text():
    ...
    try:
        if method == 'dictionary':
            ...
            results = []
            for (verb_lemma, dep_lemma), (verb, dep), similarity in errors:
                corrections = suggest_corrections(verb_lemma, dep_lemma)
                all_corrections = [f"{corr[0]} {corr[1]} (частотность в корпусе: {corr[2]})" for corr in corrections]
                frequency = check_collocation_frequency(verb_lemma, dep_lemma)
                results.append({
